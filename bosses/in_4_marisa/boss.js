    ...
    this.bossUpdate = function(player)
    {
        ...
        if(state == "1_starspin")
        {
            if(timer == 1 && !has_starspun)
            {
                ...
                for(var k = 0; k < 5; k++)
                {
                    var b = engine.makeBullet(core.x, core.y, k*72, 0, DelaySpinSpawner, image_prefix+"spellcircle.png");
        ...
        if(state == "3_trispin")
        {
            if(timer == 1 && !has_trispun)
            {
                ...
                for(var k = 0; k < 5; k++)
                {
                    var b = engine.makeBullet(core.x, core.y, k*72, 0, TriSpinSpawner, image_prefix+"spellcircle.png");
                    b.bulletclass.setParams(k)

                    var b = engine.makeBullet(core.x, core.y, k*72, 0, TriSpinSpawnerSmall, image_prefix+"spellcircle.png");
                    b.bulletclass.setParams(k)
                }
            }
        ...
        if(state == "4_eventhorizon")
        {
            if(timer == 1)
            {
                ...
                for(var k = 0; k < 10; k++)
                {
                    var b = engine.makeBullet(core.x, core.y, k*36, 0, TwistSpawner, image_prefix+"spellcircle.png");
                    b.bulletclass.setParams(k)
                }
            }
        }
        ...
        if(state == "7_doublespin")
        {
            if(timer == 1)
            {
                if(!has_7doubled)
                {
                    ...
                    for(var k = 0; k < 12; k++)
                    {
                        var b = engine.makeBullet(core.x, core.y, k*30, 0, DoubleSpinSpawner, image_prefix+"spellcircle.png");
                    }
                    has_7doubled = true;
                }
            }
...
{
    this.hitbox = new HitBoxRotatedRect(350, 10, bullet.direction);
    this.kind = 0.5;

    var dir = 0;
    var radius = 600;
    var radius_small = 450;
    var hitbox = this.hitbox;
    var movement = 0;

    var timer = 0;

    var circle = engine.makeBullet(engine.bosscore.x + Math.cos(dir/180 * Math.PI)*radius_small, engine.bosscore.y + Math.sin(dir/180 * Math.PI)*radius_small, 0, 0, Star(2, 3), image_prefix+"spellcircle.png");
    ...
    this.update = function()
    {
        ...
        engine.setBulletPosition(circle, engine.bosscore.x + Math.cos(dir/180 * Math.PI)*radius_small, engine.bosscore.y + Math.sin(dir/180 * Math.PI)*radius_small);
